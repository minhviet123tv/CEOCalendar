...
public class UpdateNoteFragment extends Fragment implements EasyPermissions.PermissionCallbacks{
...
//        loadRecyclerViewNoteImage(imageNoteArrayList);
    ...
    public void loadRecyclerViewNoteImage(ArrayList<ImageNote> imageNoteArrayList){
        ...
        ImageNoteAdapter imageNoteAdapter = new ImageNoteAdapter(view.getContext(), imageNoteArrayList, showIconDeleteOneImage, (ImageNoteAdapter.IClickAddImageIcon) () -> {
            ...
            //B. Sự kiện khi click vào item (ảnh) của rcv
            if(!showIconDeleteOneImage){

                //Gửi dữ liệu vào shared
                ViewImageShared viewImageShared = new ViewImageShared(id_image_note, id_note, position);
                UserUltils.saveViewImageLocal(view.getContext(), viewImageShared);

                //Mở menu fragment view ảnh, load lại viewPager của fragment mở ra này (Vì nếu không load lại nó sẽ load cái cũ)
                NotesViewPagerFragment.setCurrentItemNotes(4);
                ViewImageNoteViewPagerFragment.loadViewPagerViewImage();
