...
public class CreateNewNoteFragment extends Fragment implements EasyPermissions.PermissionCallbacks{
    private View view;
    private TextView txt_title_note_activity;
    private EditText edt_new_note_name, edt_content_new_note;
    private ImageButton ibtn_clear_content_new_note, ibtn_copy_note_content, ibtn_check_content_new_note, ic_back_new_note;
    private RecyclerView rcv_image_of_note;
    private ImageView img_delete_all_image_new_note, img_check_add_image_new_note, img_view_avatar_new_note;
    private RelativeLayout layout_icon_control_image_new_note;
    private Button btn_save_new_note, btn_cancel_new_note;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.fragment_create_new_note, container, false);

        //1. Ánh xạ
        AnhXa();

        //2. Load icon and focus editText
        LoadIconAndFocusEditText();

        //3. Thêm avatar: Hỏi quyền truy cập folder ảnh và thực hiện theo code
        img_view_avatar_new_note.setOnClickListener(v -> {

            //Đặt trạng thái 2 boolean chọn ảnh
            isClickSelectAvatar = true;
            isClickSelectImageList = false;

            //Ẩn các layout control khác và clear focus của editText
        ...
        //5. Back
        ic_back_new_note.setOnClickListener(v -> {
            cancelNewNote();
        });

        //6. Cancel new note
        btn_cancel_new_note.setOnClickListener(v -> {
            cancelNewNote();
        });
    ...
    private void AnhXa() {
        ...
        ic_back_new_note = view.findViewById(R.id.ic_back_new_note);
        ...
        btn_cancel_new_note = view.findViewById(R.id.btn_cancel_new_note);
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode == RESULT_OK && data != null){
            ...
            if(requestCode == FilePickerConst.REQUEST_CODE_PHOTO && isClickSelectAvatar){
                ...
                //5. Nếu có ảnh thì xác định là sẽ có avatar
    ...
    public void loadRecyclerViewNoteImage(ArrayList<ImageNote> imageNoteArrayList){
        ...
        ImageNoteAdapter imageNoteAdapter = new ImageNoteAdapter(view.getContext(), imageNoteArrayList, showIconDeleteOneImage, (ImageNoteAdapter.IClickAddImageIcon) () -> {
            ...
            //Sự kiện khi click vào item của rcv - một ảnh (đang tạo note mới -> thông báo lưu note)
            if(UserUltils.getUserLocal(requireActivity()).getCountLogin() <= 30) {
                Toast.makeText(view.getContext(), R.string.save_note_to_view, Toast.LENGTH_SHORT).show();
            }
...
//                .setImageSizeLimit(2200) ////Giới hạn kích thước hình ảnh có thể tìm thấy
    ...
    private void createNewNote() {
        ...
        //2. Tạo note mới (Chưa có id)
        MyNote myNote = new MyNote();
        int id_folder = UserUltils.getId_folder(view.getContext());
        myNote.setId_folder(id_folder);
        ...
        myNote.setStatus(1); // Dự kiến: 1: bình thường, 2: có mật khẩu
        myNote.setRole(1);

        //3. Lưu vào CSDL với 2 trường hợp không có avatar
        if(!hasAvatar) {
            truyVanDuLieuSQLite.createNewNote(myNote, imageNoteArrayList);
            ...
            byte[] byteArrayAvatar = new byte[0];
            ...
            //set thông số cho avatar và lưu vào CSDL cùng note
            myNoteAvatar.setNote_avatar(byteArrayAvatar);
            myNoteAvatar.setStatus(1);
            truyVanDuLieuSQLite.createNewNote(myNote, myNoteAvatar, imageNoteArrayList);

        }

        //4. Thông báo
//        Toast.makeText(this, getResources().getText(R.string.created), Toast.LENGTH_SHORT).show();
    ...
    //6.2 Clear fragment sau khi lưu hoặc bấm back, cancel
    private void clearFragment(){

        //Trả lại trạng thái như ban đầu chưa có gì cho các thành phần tạo dữ liệu
        edt_new_note_name.setText(null);
        edt_content_new_note.setText(null);
        img_view_avatar_new_note.setImageResource(R.drawable.ic_add_photo_4);
        imageNoteArrayList.clear();
        loadRecyclerViewNoteImage(imageNoteArrayList); //clear xong load lại để làm mới

        //Điều chỉnh lại trạng thái các boolean
        hasAvatar = false;
        showIconDeleteOneImage = false;
        isClickSelectAvatar = false;
        isClickSelectImageList = false;

        //Ẩn các layout control
        HiddenLayoutControlNoteEditText();
        HiddenLayoutControlImageList();
    ...
    //A. Cancel trực tiếp huỷ bỏ tạo note mới (Cần clear fragment)
    public void cancelNewNote() {
        //Clear trước khi cancel
        clearFragment();

        //back về fragment View Folder
